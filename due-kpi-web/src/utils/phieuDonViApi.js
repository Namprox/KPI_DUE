 ...
 *
 * PHẠM VI: module chỉ phủ phần việc của THƯ KÝ (tạo phiếu · nhập điểm · tổng hợp
 * · trình lên trưởng đơn vị). Server còn /duyet-dv, /duyet-truong, /chot, /mo-lai
 * nhưng chưa màn hình nào gọi tới — bổ sung hàm khi dựng màn hình cho chúng.
 */
...
export const TRANG_THAI_DV_META = {
  1: { label: 'Thư ký đang nhập', icon: 'fa-pen', bg: '#f1f5f9', color: '#475569', border: '#e2e8f0' },
  2: { label: 'Chờ Trưởng đơn vị duyệt', icon: 'fa-hourglass-half', bg: '#fffbeb', color: '#b45309', border: '#fde68a' },
  3: { label: 'Trưởng đơn vị đã duyệt', icon: 'fa-user-check', bg: '#eff6ff', color: '#1d4ed8', border: '#bfdbfe' },
  4: { label: 'Hiệu trưởng đã duyệt', icon: 'fa-circle-check', bg: '#f5f3ff', color: '#6d28d9', border: '#ddd6fe' },
  5: { label: 'Hoàn tất', icon: 'fa-lock', bg: '#ecfdf5', color: '#047857', border: '#a7f3d0' },
};

export const tenTrangThaiDonVi = (trangThai) =>
  TRANG_THAI_DV_META[trangThai]?.label || `Không xác định (${trangThai ?? '—'})`;
 ...
 */
export const nhapDiemChiTietDonVi = async (
  idChiTiet,
  { diem, nhanXet, rowVersion },
) => {
  const trong = diem === '' || diem === null || diem === undefined;
  const data = await sendJson(
    `chi-tiet-don-vi/${idChiTiet}/diem-nhap`,
    'PUT',
    {
