 ...
 */
const TienDoCham = ({ xong = 0, tong = 0, nhan = 'Tiến độ chấm', ghiChu, phu = false }) => {
  const phanTram = tong > 0 ? Math.round((xong / tong) * 100) : 0;
  const hoanTat = tong > 0 && xong >= tong;

  return (
    <div className={`cd-progress${phu ? ' cd-progress-phu' : ''}`}>
      <div className="cd-progress-head">
        <span>{nhan}</span>
        <b>
          {xong}/{tong}
          {tong > 0 && <span className="cd-progress-pct"> ({phanTram}%)</span>}
        </b>
      </div>
