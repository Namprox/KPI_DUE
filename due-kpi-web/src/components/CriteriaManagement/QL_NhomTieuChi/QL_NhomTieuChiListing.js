...
import '../../../css/CriteriaManagement/QL_NhomTieuChi.css';

const QL_NhomTieuChiListing = ({ data, onEdit, onDelete, isLoading, canManage }) => {
        ...
        >
            {isLoading ? (
                <div style={{ textAlign: 'center', padding: '50px' }}>
                    ...
                    <p style={{ marginTop: '10px', color: '#666' }}>Đang tải dữ liệu nhóm tiêu chí</p>
                </div>
            ) : data.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '60px 20px', color: '#666' }}>
                    <i className="fa-solid fa-list-check" style={{ fontSize: '60px', color: '#5dade2', marginBottom: '15px' }}></i>
                    <h3 style={{ color: '#e74c3c', margin: '0 0 10px 0' }}>Chưa có nhóm tiêu chí nào được tạo</h3>
                </div>
            ) : (
                <>
                    <table
                    ...
                    >
                        <thead>
                            <tr>
                                <th width="8%" style={{ textAlign: 'center' }}>STT</th>
                                <th width="35%">TÊN NHÓM TIÊU CHÍ</th>
                                <th width="20%">CẤP CHA (NẾU CÓ)</th>
                                <th width="12%" style={{ textAlign: 'center' }}>LOẠI NHÓM</th>
                            ...
                            {paginatedData.map((item, index) => {
                                ...
                                return (
                                    <tr key={item.IdNhom}>
                                        ...
                                        <td>
                                            <div style={{ fontWeight: '600', color: '#333' }}>{item.TenNhom}</div>
                                            {item.DiemToiDa && (
                                        ...
                                        <td style={{
                                            color: item.IdNhomCha ? '#0284c7' : '#94a3b8',
