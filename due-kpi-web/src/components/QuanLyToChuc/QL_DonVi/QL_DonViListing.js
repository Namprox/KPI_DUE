...
import "../../../css/QuanLyToChuc/QL_DonVi.css";
import { Link } from "react-router-dom";

const QL_DonViListing = ({ data, isLoading }) => {
            ...
            <table className="custom-table donvi-table">
              <thead>
                <tr>
                  ...
                  <th width="12%">TRỰC THUỘC (CẤP CHA)</th>
                ...
                {paginatedData.map((item, index) => {
                  ...
                  return (
                    <tr key={item.IdDonVi}>
                      ...
                      <td
                        style={{
                          color: item.IdDonViCha ? "#0394ddff" : "#999",
                          fontStyle: item.IdDonViCha ? "normal" : "italic",
                          fontWeight: "500",
                        }}
                      >
                        {item.TenDonViCha ||
                          (item.IdDonViCha ? "" : "(Không có)")}
                      </td>
                      ...
                      <td style={{ textAlign: "center" }}>
                        <Link
                          to={`${item.MaDonVi}/danh-sach-thanh-vien`}
                          className="donvi-user-count"
                        ...
                        >
                          ...
                          ></i>
                          <span>{item.SoNguoiDung} thành viên</span>
                        </Link>
                      </td>
                      <td style={{ textAlign: "center" }}>
                        {item.TrangThai ? (
                          <span className="donvi-status-active">
                            Đang hoạt động
                          </span>
                        ) : (
