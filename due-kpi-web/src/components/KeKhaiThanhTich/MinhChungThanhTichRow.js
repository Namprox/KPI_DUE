...
import { formatKb } from "../../utils/keKhaiThanhTichApi";
import { formatNgayGio } from "../../utils/phieuApi";

/**
 * Một tệp minh chứng gắn vào MỘT dòng kê khai thành tích.
 *
 * Song song với MinhChungKeKhaiRow nhưng khoá theo `IdMinhChungTt` và đi endpoint
 ...
 */
const MinhChungThanhTichRow = ({ mc, onXem, onTai, onXoa, dangXoa = false }) => {
  ...
  return (
    <div className="cd-mc-row">
      ...
      {onXoa && (
        <button
          type="button"
          className="cd-mc-act kkt-mc-xoa"
          onClick={() => onXoa(mc)}
          disabled={dangXoa}
...
export default MinhChungThanhTichRow;
