...
import '../../../css/OrganizationalManagement/QL_NhomGiangVien.css';

const QL_NhomGiangVienListing = ({ data, onEdit, onDelete, isLoading }) => {
    ...
    return (
        <div
            className="table-card nhom-giangvien-table-container"
            style={{
                overflowX: 'auto',
                background: '#fff',
                borderRadius: '8px',
        ...
        >
            {isLoading ? (
                <div style={{ textAlign: 'center', padding: '50px' }}>
                    ...
                    <p style={{ marginTop: '10px', color: '#666' }}>Đang tải danh sách nhóm giảng viên</p>
                </div>
            ) : data.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '60px 20px', color: '#666' }}>
                    <i className="fa-solid fa-user-group" style={{ fontSize: '60px', color: '#5dade2', marginBottom: '15px' }}></i>
                    <h3 style={{ color: '#e74c3c', margin: '0 0 10px 0' }}>Chưa có nhóm giảng viên nào</h3>
                </div>
            ) : (
                <>
                    <table
                        className="custom-table nhom-giangvien-table"
                        style={{ minWidth: isDesktop ? '800px' : '100%' }}
                    >
                        <thead>
                            <tr>
                                <th width="15%">MÃ NHÓM</th>
                                <th width="35%">TÊN NHÓM</th>
                        ...
                        <tbody>
                            {paginatedData.map((item) => (
                                <tr key={item.IdNhomGv}>
                                    <td style={{ fontWeight: 'bold', color: '#003399' }}>{item.MaNhom}</td>
                                    <td style={{ fontWeight: '600' }}>{item.TenNhom}</td>
                                    ...
                                    <td>
                                        <div style={{ display: 'flex', gap: '15px', justifyContent: 'center' }}>
                                            ...
                                            <div className="action-btn delete-btn" onClick={() => onDelete(item.IdNhomGv)} title="Xóa">
...
export default QL_NhomGiangVienListing;