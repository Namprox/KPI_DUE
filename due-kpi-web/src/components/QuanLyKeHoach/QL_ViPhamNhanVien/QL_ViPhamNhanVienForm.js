...
const QL_ViPhamNhanVienForm = ({
  ...
  const handleLoaiChange = (idLoai) => {
    ...
    // Điểm trừ luôn khởi tạo bằng mức của danh mục: chế độ cố định thì đó là
    // giá trị máy chủ sẽ ghi, chế độ tối thiểu thì đó là mức sàn.
    setFormData({
      ...formData,
      IdLoaiViPham: idLoai,
  ...
  const cheDo = cheDoCuaLoai(selectedLoai);
  const mucMacDinh = diemTruMacDinhCuaLoai(selectedLoai);
  const diemCoDinh = cheDo === CHE_DO_DIEM_TRU.CO_DINH;
  const diemToiThieu = cheDo === CHE_DO_DIEM_TRU.TOI_THIEU;
  const batBuocLyDo = canLyDoDieuChinh(selectedLoai, formData.DiemTru);
  const lyDoLen = (formData.LyDoDieuChinh || "").length;
  // Trần 70/30 của nhóm áp trên TỔNG cả năm, không phải trên một bản ghi -
  // chỉ hiển thị để người ghi nhận biết, không dùng để chặn.
  const tranNhom =
    selectedLoai?.TranDiemTru != null
      ? Number(selectedLoai.TranDiemTru)
      : selectedNhom?.TranDiemTru != null
        ? Number(selectedNhom.TranDiemTru)
        : null;
  ...
  const biChan = loaiBiChan || !!nhanVienBlockReason;
      ...
      >
        ...
        >
          <h3 style={{ margin: 0, paddingRight: "20px", lineHeight: "1.4" }}>
            {isEditing
              ? "Cập nhật ghi nhận vi phạm nhân viên"
        ...
        >
          <form id="viPhamNhanVienForm" onSubmit={onSubmit}>
            ...
            <div className="form-grid-2" style={{ marginBottom: "20px" }}>
              ...
              <div className="form-group">
                <label style={labelStyle}>
                  Nhân viên vi phạm <span className="text-red">*</span>
                </label>
                <SearchSelect
                  name="IdNhanVien"
                  ...
                  required
                />
                {formData.IdNhanVien && nhanVienBlockReason && (
                  ...
                  nhanVienDonVi && (
                    <div style={hintStyle}>
                      <i
                        ...
                        style={{ marginRight: "5px" }}
                      ></i>
                      Đơn vị chủ quản: {nhanVienDonVi.TenDonVi}
                    </div>
                  )}
              </div>
            </div>
            ...
            <div className="form-grid-2" style={{ marginBottom: "20px" }}>
              <div className="form-group">
                ...
                <input
                  ...
                  value={
                    diemCoDinh && mucMacDinh != null
                      ? String(mucMacDinh)
                      : (formData.DiemTru ?? "")
                  }
                  onChange={handleChange}
                  min={diemToiThieu && mucMacDinh != null ? mucMacDinh : 0}
                  step="0.01"
                  disabled={diemCoDinh}
                ...
                <div style={hintStyle}>
                  {diemCoDinh && mucMacDinh != null ? (
                    <>
                      Mức trừ cố định theo danh mục - máy chủ luôn ghi{" "}
                      {mucMacDinh.toFixed(2)} điểm.
                    </>
                  ) : diemToiThieu && mucMacDinh != null ? (
                    <>
                      Tối thiểu {mucMacDinh.toFixed(2)} điểm; trừ cao hơn thì
                      phải ghi căn cứ điều chỉnh.
                    </>
                  ) : mucMacDinh != null ? (
                    <>Mặc định của loại: {mucMacDinh.toFixed(2)}đ.</>
                  ) : (
                ...
                {tranNhom != null && (
                  <div style={hintStyle}>
                    <i
                      ...
                      style={{ marginRight: "5px" }}
                    ></i>
                    Nhóm này có trần {tranNhom.toFixed(2)} điểm/năm cho mỗi cá
                    nhân (áp khi tổng hợp, không cắt từng lần ghi nhận).
                  </div>
                )}
              </div>
            ...
            {/* Căn cứ điều chỉnh - máy chủ BẮT BUỘC khi trừ cao hơn mức sàn */}
            {batBuocLyDo && (
              <div className="form-group" style={{ marginBottom: "20px" }}>
                <label style={labelStyle}>
                  Căn cứ điều chỉnh <span className="text-red">*</span>
                </label>
                <textarea
                  name="LyDoDieuChinh"
                  className="form-input"
                  value={formData.LyDoDieuChinh || ""}
                  onChange={handleChange}
                  rows="2"
                  maxLength={500}
                  placeholder='VD: "Theo kết quả họp đơn vị ngày 12/03/2026"'
                />
                ...
                <div style={{ ...hintStyle, color: "#b45309" }}>
                  <i
                    ...
                    style={{ marginRight: "5px" }}
                  ></i>
                  Bạn đang trừ cao hơn mức tối thiểu{" "}
                  {mucMacDinh != null ? `${mucMacDinh.toFixed(2)} điểm` : ""} -
                  máy chủ yêu cầu ghi rõ căn cứ.
                </div>
              </div>
            )}
            ...
            {/* Minh chứng PDF - dùng chung endpoint minh-chung với vi phạm giảng viên */}
            <div className="form-group" style={{ marginBottom: "20px" }}>
              <label style={labelStyle}>
                Biên bản / Hồ sơ (PDF){" "}
...
export default QL_ViPhamNhanVienForm;
