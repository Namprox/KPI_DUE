...
const Khoi = ({ tieuDe, trangThai, children, moTa }) => (
  ...
  >
    <div
      ...
      style={{ marginBottom: moTa ? "4px" : "12px" }}
    >
      {tieuDe}
    </div>
    {moTa && (
      <div className="cd-hint" style={{ marginTop: 0, marginBottom: "12px" }}>
        {moTa}
      </div>
    )}
...
const HoSoKpiGiangVien = () => {
  ...
  const [gioGiang, setGioGiang] = useState({
    dangTai: true,
    loi: "",
    rows: [],
  });
  ...
  const taiTatCa = useCallback(async () => {
    ...
    fetchNckhGiangVien(idNv, idNam)
      ...
      .catch((error) =>
        setNckh({ dangTai: false, loi: error.message, data: null }),
      );
    ...
    fetchGioGiangTheoNam(idNam)
      ...
      .catch((error) =>
        setGioGiang({ dangTai: false, loi: error.message, rows: [] }),
      );
    ...
    fetchViPhamGiangVien({ idNam, idNhanVien: idNv })
      ...
      .catch((error) =>
        setViPham({ dangTai: false, loi: error.message, rows: [] }),
      );
    ...
    fetchDiemPhanHoiSv(idNam)
      .then(({ dotChot, items }) =>
        setPhanHoi({
          ...
          dotChot,
          item:
            items.find((x) => String(x.IdNhanVien) === String(idNv)) || null,
        }),
      )
      .catch((error) =>
        setPhanHoi({
        ...
        <>
          ...
          >
            {dinhMuc.data ? (
                  ...
                  <div className="stat-card">
                    ...
                    <div>
                      ...
                      <div className="stat-value">
                        {formatDiem(dinhMuc.data.GioGiangApDung, 1)}
                      </div>
                      <div className="cd-hint">
                        gốc {formatDiem(dinhMuc.data.GioGiangBase, 1)}
                      </div>
                    </div>
                  </div>
                  <div className="stat-card">
              ...
              <div className="cd-hint">
                Chưa cấu hình định mức cho giảng viên trong năm này.
              </div>
            )}
          </Khoi>
        </>
      ...
      {tab === "nckh" && (
        ...
        >
          {nckh.data?.HoSo ? (
            ...
            <div className="cd-hint">
              Giảng viên chưa được liên kết với hệ NCKH hoặc chưa có dữ liệu
              đồng bộ.
            </div>
          )}
        </Khoi>
          ...
          {gioGiangCuaGv.length > 0 ? (
            <>
              <div className="stat-card-grid" style={{ marginBottom: "16px" }}>
                ...
                <div className="stat-card">
                  ...
                  <div>
                    ...
                    <div className="stat-value">
                      {tongGioGiang.thucLinh.toFixed(1)}
                    </div>
                  </div>
                </div>
                <div className="stat-card">
                  ...
                  <div>
                    ...
                    <div className="stat-value">
                      {tongGioGiang.dinhMuc.toFixed(1)}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ overflowX: "auto" }}>
                <table
                  className="custom-table"
                  style={{ minWidth: "760px", fontSize: "13px" }}
                >
                  <thead>
                    <tr>
                      <th>Kỳ học</th>
                    ...
                    {gioGiangCuaGv.map((r) => (
                      <tr key={r.IdGioGiangImport}>
                        ...
                        <td style={{ textAlign: "right" }}>
                          {formatDiem(r.TongGioGiangDay, 1)}
                        </td>
                        <td style={{ textAlign: "right" }}>
                          {formatDiem(r.TongGioCvk, 1)}
                        </td>
                        <td style={{ textAlign: "right", fontWeight: 700 }}>
                          {formatDiem(r.TongGioQuiDoi, 1)}
                        </td>
                        <td style={{ textAlign: "right" }}>
                          {formatDiem(r.DinhMucGioChuan, 1)}
                        </td>
                        <td style={{ textAlign: "right" }}>
                          {formatDiem(r.TongGioThucLinh, 1)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <div className="cd-hint">
              Không tìm thấy dòng giờ giảng nào khớp họ tên giảng viên trong các
              kỳ của năm này.
            </div>
          )}
        </Khoi>
      ...
      {tab === "viPham" && (
        ...
        >
          {viPhamCuaGv ? (
              ...
              <div className="stat-card-grid">
                ...
                <div className="stat-card">
                  ...
                  <div>
                    ...
                    <div className="stat-value">
                      {formatDiem(viPhamCuaGv.TongDiemTruTho)}
                    </div>
                  </div>
                </div>
                <div className="stat-card">
                  ...
                  <div>
                    ...
                    <div className="stat-value">
                      {formatDiem(viPhamCuaGv.DiemTruCaNhan)}
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="cd-hint">
              Giảng viên không có vi phạm nào được ghi nhận trong năm này.
            </div>
          )}
        </Khoi>
      )}
      ...
      {tab === "phanHoi" && (
        ...
        >
          {phanHoi.item ? (
            <div className="stat-card-grid">
              <div className="stat-card">
                ...
                <div>
                  ...
                  <div className="stat-value">
                    {formatDiem(phanHoi.item.DiemTrungBinh)}
                  </div>
                </div>
              </div>
              <div className="stat-card">
            ...
            <div className="cd-hint">
              Năm này chưa chốt điểm phản hồi sinh viên, hoặc giảng viên không
              có trong đợt chốt.
            </div>
          )}
        </Khoi>
