...
const DanhSachPhieu = () => {
  ...
  const hienChuaLap = (trangThaiChon.length === 0 || chonChuaLap) && !dangLocNgay;
  ...
  }, [selectedNam, idDonVi, trangThaiPhieu, tuNgay, denNgay, sortBy, boQuaPhieu]);
    ...
    <div className="page-container">
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            borderColor: chonChuaLap ? TRANG_THAI_CHUA_LAP_META.border : '#e2e8f0',
          }}
        ...
        >
          ...
          {!dangTaiChuaLap && chuaLapPhieu.length > 0 && ` (${chuaLapPhieu.length})`}
        </button>
      ...
      {coTheChot && !dangTaiBang && soChoChot > 0 && (
        <div className="cd-goi-tom-tat">
          ...
          >
            <i className={`fa-solid ${metaChoChot.icon}`}></i> {metaChoChot.label}
          </span>
          <span>
            <b>{soChoChot}</b> hồ sơ khớp bộ lọc đã thẩm định xong và chờ bạn chọn
            xếp loại để chốt.
          </span>
          <button className="cd-link-btn" onClick={() => navigate('/quan-ly/duyet-ho-so')}>
      ...
      {chonChuaLap && dangLocNgay && (
        <div className="cd-canh-bao">
          ...
          <span>
            Đang lọc theo khoảng ngày nên người chưa lập phiếu bị bỏ ra: họ không có
            ngày tạo hay ngày gửi để khớp. Xóa hai ô ngày để xem lại họ.
          </span>
        </div>
      )}
      ...
      {hienChuaLap && loiChuaLap && (
        <div className="cd-canh-bao">
          ...
          <span>
            {loiChuaLap}. Bảng dưới đây chỉ còn là danh sách phiếu — người chưa lập
            phiếu KHÔNG được ghép vào, đừng coi đây là toàn cảnh đơn vị.
          </span>
        </div>
      )}
      ...
      <div className="modern-table-card">
        {dangTaiBang ? (
          ...
          <div className="cd-empty">
            <i className={`fa-solid ${boQuaPhieu ? 'fa-circle-check' : 'fa-folder-open'}`}></i>
                      ...
                      <div className="table-actions">
                        ...
                        {!p.IdPhieu ? (
                          <button
                            ...
                            onClick={() => navigate(`/quan-ly/giang-vien/${p.IdNhanVien}`)}
                          >
                        ...
                        {coTheChot && p.TrangThai === TRANG_THAI.CHO_TK_DUYET && (
                          <button
                            className="action-btn chot-btn"
                            title="Chọn xếp loại và chốt hồ sơ này"
                            onClick={() => navigate(`/quan-ly/duyet-ho-so/${p.IdPhieu}`)}
                          >
