...
const ToTrinhKhoa = () => {
  ...
  const handleTrinh = async ({ nhanXet }) => {
    ...
    setDangXuLy(true);
    try {
      await trinhToTrinh(goi.IdToTrinh, {
        nhanXet,
  ...
  const luuUuTien = async () => {
    for (const h of dongHang?.hoSo || []) {
      ...
      const rowVersion =
        h.RowVersion || (await fetchPhieuDetail(h.IdPhieu))?.RowVersion;
    ...
    <div className="page-container">
      ...
      {isLoading ? (
        ...
        <div className="modern-table-card">
          <div className="cd-empty">
            ...
            <p style={{ margin: 0 }}>
              Gói được tạo tự động khi hồ sơ đầu tiên của Khoa được Trưởng khoa
              chốt.
            </p>
          </div>
        </div>
            ...
            {goi.LyDoTraVe && (
              <div className="cd-yeu-cau-bo-sung" style={{ marginTop: "16px" }}>
                <div className="cd-yc-head">
                  ...
                  >
                    <i className="fa-solid fa-rotate-left"></i> Hiệu trưởng trả
                    về
                  </span>
                </div>
      ...
      {dongHang && (
        <div
          ...
          onClick={dangXuLy ? undefined : () => setDongHang(null)}
        >
          <div
