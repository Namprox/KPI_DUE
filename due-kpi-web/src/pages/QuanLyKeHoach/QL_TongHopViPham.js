...
const QL_TongHopViPham = () => {
  ...
  const stats = useMemo(() => {
    ...
    const khoaBiTru = khoaRows.filter(
      (k) => Number(k.DiemTruTapThe || 0) > 0,
    ).length;
    ...
    try {
      ...
      if (gvRes.ok) {
        ...
        const err = await readApiError(
          gvRes,
          "Không tải được tổng hợp cá nhân",
        );
      ...
      if (khoaRes.ok) {
        ...
        const err = await readApiError(
          khoaRes,
          "Không tải được điểm trừ tập thể Khoa",
        );
        showToast("error", "Lỗi", err.message);
        setKhoaRows([]);
      }
    ...
    if (khoaRows.length > 0) {
      downloadExcel({
        data: khoaRows.map((r, i) => ({
          ...
          "Tổng điểm trừ cá nhân (T)":
            r.TongDiemTruCaNhan != null ? Number(r.TongDiemTruCaNhan) : 0,
          "Mẫu số (0.2 x 15 x N)": r.MauSo != null ? Number(r.MauSo) : 0,
          "Điểm trừ tập thể (trần 7.5)":
            r.DiemTruTapThe != null ? Number(r.DiemTruTapThe) : 0,
        })),
    ...
    <div className="page-container" style={{ padding: "20px" }}>
      ...
      >
        <div>
          ...
          >
            Điểm trừ KPI cá nhân (trần 15 điểm) và điểm trừ KPI tập thể của Khoa
            (trần 7,5 điểm)
          </p>
        </div>
